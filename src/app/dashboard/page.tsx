...
interface Profile { business_name: string; business_type: string; owner_name: string; id: string; }
...
function DashboardContent() {
  ...
  const isNew = searchParams.get("setup") === "1";
  ...
  function copyEmbed() {
    const code = `<script src="${appUrl}/widget.js" data-business="${profile?.id}"></script>`;
  ...
  const embedCode = `<script src="${appUrl}/widget.js" data-business="${profile?.id}"></script>`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
        {isNew && (
          <div style={{ background: "#6b8cff1a", border: "1px solid #6b8cff44", borderRadius: 16, padding: 24, marginBottom: 32, textAlign: "center" }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>🎉</div>
            <div style={{ fontWeight: 800, fontSize: 20, marginBottom: 8 }}>Welcome to Apex AI Receptionist!</div>
            <div style={{ color: "#888", fontSize: 14 }}>Your free trial has started. Configure your AI below and paste the embed code on your website.</div>
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 28 }}>
          <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 16, padding: 24, borderTop: `2px solid ${statusColor}` }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: statusColor, textTransform: "capitalize" }}>{subscription?.status || "No Plan"}</div>
        ...
        <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 16, padding: 28, marginBottom: 24 }}>
          ...
          <div style={{ color: "#555", fontSize: 13, marginBottom: 16 }}>
            Paste this one line into your website HTML, right before the <code style={{ color: "#c084fc", background: "#1e1e1e", padding: "1px 6px", borderRadius: 4 }}>&lt;/body&gt;</code> tag.
          </div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 16 }}>
            <button onClick={copyEmbed} style={{ padding: "9px 18px", background: copied ? "#1a2e1a" : "#1e1e1e", border: "1px solid #2a2a2a", borderRadius: 8, color: copied ? "#4ade80" : "#888", fontSize: 13, cursor: "pointer" }}>
              {copied ? "✓ Copied!" : "Copy Code"}
            </button>
            <a href={`/widget/${profile?.id}`} target="_blank" rel="noreferrer" style={{ padding: "9px 18px", background: "#6b8cff1a", border: "1px solid #6b8cff44", borderRadius: 8, color: "#6b8cff", fontSize: 13, textDecoration: "none" }}>Preview Widget →</a>
          </div>
        </div>
        ...
        <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 16, padding: 28 }}>
          ...
          {[
            ...
            { done: false, label: "Paste embed code on website", desc: "One line of code, done in 2 minutes" },
            { done: subscription?.status === "active" || subscription?.status === "trialing", label: "Subscription active", desc: "7-day free trial" },
          ].map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: i < 4 ? "1px solid #111" : "none" }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: item.done ? "#1a2e1a" : "#1e1e1e", border: `2px solid ${item.done ? "#4ade80" : "#2a2a2a"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0, color: item.done ? "#4ade80" : "#555" }}>
                {item.done ? "✓" : i + 1}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ color: item.done ? "#666" : "#ddd", fontWeight: 600, fontSize: 14, textDecoration: item.done ? "line-through" : "none" }}>{item.label}</div>
                <div style={{ color: "#444", fontSize: 12 }}>{item.desc}</div>
              </div>
              {"link" in item && !item.done && item.link && (
                <a href={item.link} style={{ color: "#6b8cff", fontSize: 13, textDecoration: "none", fontWeight: 600 }}>Set up →</a>
              )}
            </div>
          ))}
        </div>
